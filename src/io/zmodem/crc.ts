export class Crc16 {
  ...
  private checkCrc() {
    if (this.value < 0 || this.value > 0xfffff)
      throw new Error(`crc: ${this.value} out of range`);
  }
  ...
  public update(b: number): void {
    this.checkCrc();
    this.checkByte(b);
    const idx = (this.value >> 8) & 0xff;
    const val = (Crc16.CRC_TABLE[idx] as number) ^ (this.value << 8) ^ b;
