...
export class Numeric {
  private static getBase(val: string): Base {
    ...
    if (val.match(hexWithPrefix)) return Base.Hexadecimal;

    const decimalWithPrefix = "^0[dD][0-9]+";
    if (val.match(decimalWithPrefix)) return Base.Decimal;

    const octalWithPrefix = "^0[oO]?[0-7]+$";
    if (val.match(octalWithPrefix)) return Base.Octal;

    const decimalWithoutPrefix = "^[0-9]+$";
    if (val.match(decimalWithoutPrefix)) return Base.Decimal;
